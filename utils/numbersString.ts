...
export function toFixedS(string: string | number | bigint, decimals: number) {
  ...
  const decimalPart = string.substring(
    indexOfDot + 1,
 ...
 */
export function moveDecimalDot(
  ...
  const decimalPart =
    indexOfDot === -1 ? "" : string.substring(indexOfDot + 1, string.length);
  ...
  if (positions > 0) {
    return (
      ...
      (positions >= decimalPart.length
        ? "0".repeat(positions - decimalPart.length)
        : "")
    );
  } else {
    return (
      intPart.substring(0, intPart.length + positions) +
      (intPart.length <= Math.abs(positions)
        ? `0.${"0".repeat(Math.abs(positions) - intPart.length)}`
        : ".") +
